...
# TODO Handle hashers that have optional inputs, ie. blake and shake
# TODO calc multiple hashes from one input.


def hash_a_string(
    string: str, hasher: Any, as_hex_str: bool = False, encoding: str = "utf-8"
    ...
    Args:
        ...
        hasher: The hash function from `hashlib`
        as_hex_str: Return the digest as bytes, or a hexidecimal string, defaults to False
        encoding: The encoding used to convert the string to bytes. Defaults to "utf-8".

    Returns:
        [type]: [description]
    """
    hasher.update(string.encode(encoding))
